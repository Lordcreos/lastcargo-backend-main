...
const getAllTrans = async (scraper) => {
  const existingTrans = await strapi.query('transporte').find({
    _limit: 1000,
    scraper: scraper.id
  }, ["titulo"]);
  const allTrans = existingTrans.map(x => x.titulo);
  console.log(`Transporte Logistica in database: \t${chalk.blue(allTrans.length)}`);
  return allTrans;
}
...
module.exports = { scraperCanRun, getAllSG, getDate, getReport , getAllTrans}
